...
<div class="text-xl text-gray-400">下次 AI 說「我做完了」</div>
...
第一，問題不在 AI 不夠強。換更強的模型不會解決這件事，因為從頭到尾沒有人定義過什麼叫完成。

第二，SDD 就是先定義完成。驗收標準、EARS 句型，都是為了把 AI 的猜測空間壓掉。

第三，OpenSpec 適合我們。specs 跟 changes 分離、brownfield-first、純 Markdown、不需要 API key，這四點剛好對上我們的處境。

第四，但它不是銀彈。LLM 是非確定性的，自然語言天生曖昧，不要期待「規格寫好，AI 就自動做對」。

第五，也是我最想留給大家的——真正的收穫是脈絡。為什麼這樣決定、為什麼不用另一種做法，這些東西現在只活在某個人腦裡，spec 讓它們留在 repo 裡。

所以，下次 AI 跟你說「我做完了」，反問它一句：完成了什麼？

如果你答得出來，代表你的規格有寫好。如果答不出來，那就是今天這套流程可以幫上忙的地方。
